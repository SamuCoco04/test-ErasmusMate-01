...
const ensureEditable = (item: ContentItem, actorId: string) => {
...
export function useSocialStoreState() {
  useEffect(() => {
    socialStore.hydrate();
  }, []);

  return useSyncExternalStore(socialStore.subscribe, socialStore.getState, socialStore.getState);
}
