  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">Official institutional overview of submissions, deadlines, and exceptions.</p>
      </div>
          ...
          {actionItems.map((item) => (
            <div key={item.id} className="flex items-center justify-between rounded-lg border bg-white p-3">
              <div>
                ...
                <p className="text-sm text-muted-foreground">{item.id} · Due {item.dueDate}</p>
              </div>
              <div className="flex items-center gap-2">
                <Badge className={statusStyle[item.state]}>{item.state}</Badge>
                <Link href={`/student/submissions/${item.id}`} className="text-sm text-blue-700 underline underline-offset-2">Open</Link>
              </div>
            </div>
          ))}
        </CardContent>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Effective dates account for approved and applied exception overrides.</CardDescription>
        </CardHeader>
        <CardContent className="space-y-3">
